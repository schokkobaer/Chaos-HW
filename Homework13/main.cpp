...
// Resolves scene resource paths (e.g. bitmap texture files) that are given relative to the Homework13 folder.
std::string resolveResourcePath(const std::string& relativePath) {
	...
	const std::vector<std::filesystem::path> candidates = {
		std::filesystem::path("homework13") / input,
		std::filesystem::path("../homework13") / input,
		std::filesystem::path("../../homework13") / input,
	};
...
int main(int argc, char** argv) {
	...
	std::filesystem::create_directories("output/homework13");
	...
	for (const std::string& sceneFile : sceneFiles) {
		...
		const std::filesystem::path scenePath(resolvedSceneFile);
			...
			const std::string outputPath = "output/homework13/" + scenePath.stem().string()
				+ ".ppm";
