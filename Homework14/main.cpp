...
// Resolves scene resource paths (e.g. bitmap texture files) that are given relative to the homework14 folder.
std::string resolveResourcePath(const std::string& relativePath) {
	...
	const std::vector<std::filesystem::path> candidates = {
		std::filesystem::path("homework14") / input,
		std::filesystem::path("../homework14") / input,
		std::filesystem::path("../../homework14") / input,
	};
...
int main(int argc, char** argv) {
	...
	std::filesystem::create_directories("output/homework14");
	...
	for (const std::string& sceneFile : sceneFiles) {
		...
		const std::filesystem::path scenePath(resolvedSceneFile);
			...
			const std::string outputPath = "output/homework14/" + scenePath.stem().string()
				+ ".ppm";
